...
class ChangeLanguageSpec extends PlaySpec with GuiceOneAppPerSuite with LanguageHelper with ConfigHelper {
  ...
  "change language page" should {
    ...
    "render the correct content in english" in {
      val document =
        Jsoup.parse(template(form, Call("/myurl", "GET"))(authRequest, messagesInEnglish()).toString())
      document
    ...
    "render the correct content in welsh" in {
      val document =
        Jsoup.parse(template(form, Call("/myurl", "GET"))(authRequest, messagesInWelsh()).toString())
