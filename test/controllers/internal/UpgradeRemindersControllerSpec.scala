...
class UpgradeRemindersControllerSpec extends UnitSpec with MockitoSugar with WithFakeApplication {
  ...
  "the posting upgrade form" should {
    "redirect to thank you page with supplied redirect url if digital button is pressed and t&cs checked" in new UpgradeTestCase {
      ...
      when(controller.entityResolverConnector.updateTermsAndConditions(is(Generic -> TermsAccepted(true)), email = is(None))(any())).thenReturn(Future.successful(PreferencesCreated))

      val result = await(controller._upgradePreferences("someUrl", authContext)(testRequest.withFormUrlEncodedBody("opt-in" -> "true", "accept-tc" -> "true")))
    ...
    "return the page with an error if if digital button is pressed but t&cs unchecked" in new UpgradeTestCase {
      ...
      val result = await(controller._upgradePreferences("someUrl", authContext)(testRequest.withFormUrlEncodedBody("opt-in" -> "true", "accept-tc" -> "false")))

      status(result) shouldBe 400
      Jsoup.parse(contentAsString(result)).select(".error-notification").text shouldBe "You must accept the terms and conditions"
      verifyZeroInteractions(controller.entityResolverConnector)
    }

    "redirect to supplied url if non-digital button pressed " in new UpgradeTestCase {

      when(controller.entityResolverConnector.updateTermsAndConditions(is(Generic -> TermsAccepted(false)), email = is(None))(any())).thenReturn(Future.successful(PreferencesCreated))

      val result = await(controller._upgradePreferences("someUrl", authContext)(testRequest.withFormUrlEncodedBody("opt-in" -> "false")))
    ...
    "redirect to supplied url when digital true and no preference found" in new UpgradeTestCase {
      when(controller.entityResolverConnector.updateTermsAndConditions(is(Generic -> TermsAccepted(true)), email = is(None))(any())).thenReturn(Future.successful(PreferencesFailure))
    ...
    "redirect to supplied url when digital false and no preference found" in new UpgradeTestCase {
      when(controller.entityResolverConnector.updateTermsAndConditions(is(Generic -> TermsAccepted(false)), email = is(None))(any())).thenReturn(Future.successful(PreferencesFailure))
  ...
  "the upgrade page" should {
    ...
    "redirect to supplied url when no preference found" in new UpgradeTestCase {
      when(controller.entityResolverConnector.getPreferences()(any())).thenReturn(Future.successful(None))

      val result = await(controller._renderUpgradePageIfPreferencesAvailable(authContext, Encrypted("someUrl"))(testRequest))
  ...
  trait UpgradeTestCase  {
    ...
    val authContext = AuthContext(authority = ninoAndPayeAuthority("userId", utr.value, nino.value), nameFromSession = Some("Ciccio"), governmentGatewayToken = None)
    ...
    def upgradeAndCaptureAuditEvent(digital: (TermsType, TermsAccepted)):ArgumentCaptor[ExtendedDataEvent] = {
      when(controller.entityResolverConnector.updateTermsAndConditions(is(digital), email = is(None))(any())).thenReturn(Future.successful(PreferencesExists))
      await(controller.upgradePaperless(authContext, digital))
