...
object AuthorityUtils {
  ...
  def payeAuthority(id: String, nino: String): Authority =
    Authority(s"/auth/oid/$id", Accounts(paye = Some(PayeAccount(s"/paye/individual/$nino", Nino(nino)))), None, None, credentialStrength = CredentialStrength.Strong, confidenceLevel = ConfidenceLevel.L50)

  def ninoAndPayeAuthority(id: String, utr: String, nino: String): Authority =
    Authority(s"/auth/oid/$id", Accounts(sa = Some(SaAccount(s"/sa/individual/$utr", SaUtr(utr))), paye = Some(PayeAccount(s"/paye/individual/$nino", Nino(nino)))), None, None, credentialStrength = CredentialStrength.Strong, confidenceLevel = ConfidenceLevel.L50)
