...
object OptInTaxCreditsDetailsForm {
    ...
    Form[Data](
      mapping(
        ...
        "termsAndConditions" -> tuple(
          ...
          {
            case (paperlessChoice: Option[Boolean], acceptTc: Option[Boolean]) =>
              (paperlessChoice.map(PaperlessChoice.fromBoolean(_)), acceptTc)
          }
        ).verifying(
          "tc_printing_preference.accept_tc_required",
          kvp => ((kvp._1.contains(true) && kvp._2.contains(true)) || !kvp._1.contains(true))
        )
      )(Data.apply)(Data.unapply)
  ...
  case class Data(
    ...
    emailVerified: Option[String],
    emailAlreadyStored: Option[Boolean],
    termsAndConditions: (Option[Boolean], Option[Boolean])
  ) {
    lazy val isEmailVerified = emailVerified.contains("true")
    lazy val isEmailAlreadyStored = emailAlreadyStored.contains(true)
  ...
  object Data {
    ...
    def apply(
      emailAddress: Option[EmailAddress],
      termsAndConditions: (Option[PaperlessChoice], Option[Boolean]),
      emailAlreadyStored: Option[Boolean]
    ): Data = {
      val emailAddressAsString = emailAddress.map(_.value)
      val optedInAsBoolean = termsAndConditions._1.map(_.toBoolean)
      Data(
        (emailAddressAsString, emailAddressAsString),
        None,
        emailAlreadyStored,
        (optedInAsBoolean, termsAndConditions._2)
      )
    }
  }
}
