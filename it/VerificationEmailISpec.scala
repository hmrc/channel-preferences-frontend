...
class VerificationEmailISpec extends PreferencesFrontEndServer {
  ...
  "Attempt to verify a change of address with an old link" should {
    ...
    "display expired old email address message if another old email is verified and the new email is verified" in new VerificationEmailTestCase {
      ...
      `/preferences/terms-and-conditions`(ggAuthHeaderWithUtr).postGenericOptIn(firstEmail) should have(status(201))
      ...
      `/preferences`(ggAuthHeaderWithUtr).putPendingEmail(secondEmail) should have(status(200))
      clearEmails()
      `/preferences`(ggAuthHeaderWithUtr).putPendingEmail(newEmail) should have(status(200))
