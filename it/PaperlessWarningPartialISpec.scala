...
class PaperlessWarningPartialISpec extends EmailSupport {
  ...
  "Paperless warnings partial " should {
    ...
    "be not found if the user has no preferences with utr only" in new TestCase {
      val response = `/paperless/warnings`.withHttpHeaders(cookieWithUtr).get().futureValue

      response.status must be(404)
    }

    "be not found if the user has no preferences with nino only" in new TestCase {
      val response = `/paperless/warnings`.withHttpHeaders(cookieWithNino).get().futureValue
