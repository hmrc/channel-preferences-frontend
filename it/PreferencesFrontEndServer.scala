...
trait TestCaseWithFrontEndAuthentication extends TestCase {

  implicit val hc = HeaderCarrier()
  val authHelper = app.injector.instanceOf[AuthHelper]
  lazy val ggAuthHeaderWithUtr = authHelper.authHeader(utr)
  lazy val ggAuthHeaderWithNino = authHelper.authHeader(nino)

  lazy val cookieWithUtr = ggAuthHeaderWithUtr
  lazy val cookieWithNino = ggAuthHeaderWithNino
  ...
  def `/paperless/activate-from-token/:svc/:token`(svc: String, token: String) = new {
    ...
    val url = wsUrl(s"/paperless/activate-from-token/$svc/$token")
      .withHttpHeaders(("Authorization", authHelper.authorisedTokenFor().futureValue))
      .withQueryString(
  ...
  def `/paperless/activate`(
    ...
    new {
      ...
      //val token = authHelper.authorisedTokenFor(taxIdentifiers.head).futureValue
