...
object MicroServiceConfig {
  ...
  lazy val ggwServiceUrl = s"$protocol://${Play.configuration.getString(s"govuk-tax.$env.services.ggw.host").getOrElse("localhost")}:${Play.configuration.getInt(s"govuk-tax.$env.services.ggw.port").getOrElse(8550)}"
