...
//
//  private abstract class WithPayeSetup extends WithApplication(FakeApplication()) {
//    val controller = new ()
//  }
//
//
//  private abstract class WithBiztaxSetup extends WithApplication(FakeApplication()) {
...
//
//    val session = Map(
//      SessionKeys.sessionId -> Some(s"session-${UUID.randomUUID}"),
//      SessionKeys.userId -> Some(user.userId),
