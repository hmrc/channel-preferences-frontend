...
class CarAndFuelSpec extends BaseSpec {
  ...
  val defaultCar = Car(dateCarRegistered = Questionable(new LocalDate(), true))
  val carWithWithdrawnDate = defaultCar.copy(dateCarWithdrawn = Some(new LocalDate()))
  
  "CarAndFuel" should {
    "be active if the Car benefit does not have a withdrawn date" in {
      val carBenefit = Benefit(BenefitTypes.CAR, 2013, 100, 1, car = Some(defaultCar))
      CarAndFuel(carBenefit).isActive shouldBe true
    }

    "be inactive if the Car benefit has a withdrawn date" in {
      val carBenefit = Benefit(BenefitTypes.CAR, 2013, 100, 1, car = Some(carWithWithdrawnDate))
