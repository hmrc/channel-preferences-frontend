    ...
    implicit request =>
      ...
      encodedGovernmentGatewayToken match {
        case Some(token) => {
          ...
          val destinationUrl = retriveDestinationUrl(request)
          val encryptedPayload = SsoPayloadEncryptor.encrypt(generateJsonPayload(encodedGovernmentGatewayToken, destinationUrl))
          Ok(encryptedPayload)
        }
