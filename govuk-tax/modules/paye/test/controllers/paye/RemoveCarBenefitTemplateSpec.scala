...
class RemoveCarBenefitTemplateSpec extends PayeBaseSpec {
  ...
  "the remove car benefit form" should {
    val form = updateRemoveCarBenefitForm(None, new LocalDate(), None, CarFuelBenefitDates(None, None), dateToday, taxYearInterval)
  ...
  "Given a user who has car and fuel benefits, removing car benefit " should {
    ...
    val form = updateRemoveCarBenefitForm(None, new LocalDate(), Some(FuelBenefit(new LocalDate(), 0, 0, Some(new LocalDate()))), CarFuelBenefitDates(None, None), dateToday, taxYearInterval)
