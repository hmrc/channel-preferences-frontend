...
class PayeHomeControllerSpec
  ...
  "The home method" should {
    ...
    "show the first login message " in new WithApplication(FakeApplication()) {
      val content = requestHomeAction()
      Jsoup.parse(content).select("#lastlogin").text shouldBe "John Densmore, this is the first time you have logged in"
    }
    ...
    def requestHomeAction(user: User = johnDensmore): String = {
      val result = Future.successful(controller.homeAction(FakeRequest())(user))
