...
trait PayeBaseSpec extends BaseSpec  with TaxYearSupport{
  ...
  val johnDensmoresRemovedCarBenefitFormData = new RemoveCarBenefitFormData(LocalDate(2014, 1, 8), Some(true), Some(10), Some(true), Some(2000), Some("differentDateFuel"), Some(LocalDate(testTaxYear, 2, 8)))
