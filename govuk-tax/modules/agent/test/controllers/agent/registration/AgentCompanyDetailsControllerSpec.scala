...
class AgentCompanyDetailsControllerSpec extends BaseSpec with MockitoSugar {
  ...
  "The company details page" should {
    ...
    "go to next step if required details are provided" in new WithApplication(FakeApplication()) {
      ...
      keyStoreData(companyName) must be("Alvaro Ltd")
      keyStoreData(tradingName) must be("Alvarito")
      keyStoreData(qualifiedLandlineNumber) must be("1234")
      keyStoreData(qualifiedMobileNumber) must be("5678")
      keyStoreData(website) must be("alvarito.com")
      keyStoreData(email) must be("alvaro@alvaro.com")
      keyStoreData(mainAddress + '.' + addressLine1) must be("Main line 1")
      keyStoreData(mainAddress + '.' + addressLine2) must be("Main line 2")
      keyStoreData(mainAddress + '.' + addressLine3) must be("Main line 3")
      keyStoreData(mainAddress + '.' + addressLine4) must be("Main line 4")
      keyStoreData(mainAddress + '.' + postcode) must be("E33BA")
      keyStoreData(communicationAddress + '.' + addressLine1) must be("Comm line 1")
      keyStoreData(communicationAddress + '.' + addressLine2) must be("Comm line 2")
      keyStoreData(communicationAddress + '.' + addressLine3) must be("Comm line 3")
      keyStoreData(communicationAddress + '.' + addressLine4) must be("Comm line 4")
      keyStoreData(communicationAddress + '.' + postcode) must be("E33BB")
      keyStoreData(businessAddress + '.' + addressLine1) must be("Company line 1")
      keyStoreData(businessAddress + '.' + addressLine2) must be("Company line 2")
      keyStoreData(businessAddress + '.' + addressLine3) must be("Company line 3")
      keyStoreData(businessAddress + '.' + addressLine4) must be("Company line 4")
      keyStoreData(businessAddress + '.' + postcode) must be("E33BC")
      keyStoreData(saUtr) must be("7453627123")
      keyStoreData(ctUtr) must be("ctValue")
      keyStoreData(vatVrn) must be("vatvrnValue")
      keyStoreData(payeEmpRef) must be("payempValue")
      keyStoreData(companyHouseNumber) must be("764536")
      keyStoreData(registeredOnHMRC) must be("true")
    }
  }
