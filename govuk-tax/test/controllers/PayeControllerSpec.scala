...
import microservice.paye.domain.PayeRoot
import microservice.paye.CalculationResult
import scala.Some
import microservice.paye.domain.Employment
import microservice.auth.domain.Regimes
import microservice.paye.domain.Car
import play.api.test.FakeApplication
import microservice.paye.domain.Benefit
import microservice.paye.domain.TaxCode
...
class PayeControllerSpec extends BaseSpec with ShouldMatchers with MockitoSugar with CookieEncryption {
  ...
  "The benefits list page" should {
    ...
    def requestBenefits(id: String) = {
      val result = controller.listBenefits(FakeRequest().withSession("userId" -> encrypt(s"/auth/oid/$id"), sessionTimestampKey -> controller.now().getMillis.toString))
