...
class SaMicroServiceSpec extends WordSpec with MockitoSugar with ShouldMatchers with BeforeAndAfterEach {
  ...
  "SaMicroService" should {
    "save preferences for a user that wants email notifications" in new WithApplication(FakeApplication()) {
      ...
      verify(preferenceMicroService.httpWrapper).post(Matchers.eq(s"/preferences/sa/utr/$utr/print-suppression"), bodyCaptor.capture(), Matchers.any[Map[String, String]])
    ...
    "get preferences for a user who opted for email notification" in new WithApplication(FakeApplication()) {
      ...
      when(preferenceMicroService.httpWrapper.get[SaPreference](s"/preferences/sa/utr/$utr/print-suppression")).thenReturn(Some(SaPreference(true, Some("someEmail@email.com"))))
      val result = preferenceMicroService.getPreferences(utr).get
      verify(preferenceMicroService.httpWrapper).get[SaPreference](s"/preferences/sa/utr/$utr/print-suppression")
    ...
    "get preferences for a user who opted for paper notification" in new WithApplication(FakeApplication()) {
      ...
      when(preferenceMicroService.httpWrapper.get[SaPreference](s"/preferences/sa/utr/$utr/print-suppression")).thenReturn(Some(SaPreference(false)))
      val result = preferenceMicroService.getPreferences(utr).get
      verify(preferenceMicroService.httpWrapper).get[SaPreference](s"/preferences/sa/utr/$utr/print-suppression")
    ...
    "return none for a user who has not set preferences" in new WithApplication(FakeApplication()) {
      ...
      when(preferenceMicroService.httpWrapper.get[SaPreference](s"/preferences/sa/utr/$utr/print-suppression")).thenThrow(new MicroServiceException("Not Found", mockPlayResponse))
